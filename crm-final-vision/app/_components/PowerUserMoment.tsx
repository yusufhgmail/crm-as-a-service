...
export function PowerUserMoment() {
  ...
  return (
    <div className="power-user-window">
      <div className="power-user-topbar"><div><span className="mini-coremark" aria-hidden="true"><i /></span><strong>CRM From Within</strong></div><small>Question for Maya · Power user</small></div>
      <div className="power-user-thread" aria-live="polite">
        <div className="ai-message"><span>CF</span><p>I noticed that you moved Northstar into onboarding before the agreement was signed. What were you trying to accomplish?</p></div>
        {selected === null ? (
          ...
          <>
            ...
            <div className="captured-idea"><span>IMPROVEMENT CAPTURED</span><strong>{answers[selected][1]}</strong><p>CRM From Within will suggest the change for review. It will not alter the workflow silently.</p><button type="button" onClick={() => setSelected(null)}>Try another answer</button></div>
          </>
        ) : (
