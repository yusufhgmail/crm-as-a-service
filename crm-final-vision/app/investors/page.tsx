...
export const metadata: Metadata = {
  ...
  description: 'How CRM From Within aims to make every CRM installation improve the next one—and the installations already running.',
};
    ...
    <main className="investor-page">
      ...
      <section className="investor-hero shell" id="top">
        <div className="investor-hero-copy">
          ...
          <h1>Every CRM should teach us how to build the next one.</h1>
        ...
        <div className="investor-system-preview" aria-label="Three private CRM installations send approved lessons to CRM From Within's shared learning system and receive relevant improvements">
      ...
      <section className="section flywheel-section shell" id="flywheel">
        <div className="section-heading centered-heading"><p className="eyebrow">Why this can compound</p><h2>Every useful CRM strengthens two flywheels.</h2><p>The customer receives software that becomes more fitted over time. CRM From Within improves the method and lowers the effort required for the next build.</p></div>
      ...
      <section className="section market-section">
        <div className="shell">
          <div className="section-heading split-heading"><div><p className="eyebrow light">The market has already removed one technical doubt</p><h2>Email can populate and maintain a CRM. That is no longer the whole opportunity.</h2></div><p>Current products already reduce logging and infer records from conversations. CRM From Within must prove the harder step: custom software the customer owns, intent captured in the moment and learning that improves every installation.</p></div>
          ...
          <div className="market-position"><span>CRM FROM WITHIN’S NARROWER BET</span><strong>The company should not only receive an automatically populated CRM. It should receive owned software whose structure and abilities keep being rebuilt around what that company is trying to accomplish.</strong></div>
        </div>
      </section>

      <section className="section founder-section shell">
        <div className="founder-mark">YY</div><div><p className="eyebrow">Why start with CRM</p><h2>The domain knowledge comes before the automation.</h2></div><p>Yusuf Young founded FunnelBud in 2015 after implementing HubSpot and Salesforce systems. FunnelBud says it has helped more than 450 Swedish companies. That supports the choice of CRM as the first repeatable process; it does not prove the CRM From Within product or flywheel.</p><a href="https://www.funnelbud.com/en/om-oss/" target="_blank" rel="noreferrer">See the public FunnelBud record ↗</a>
      </section>
      ...
      <section className="section proof-section" id="proof">
        <div className="shell">
          ...
          <div className="failure-test"><span>THE FAILURE TEST</span><strong>If customer count rises but build time, correction rate, customer usefulness and existing-installation improvement do not get better, CRM From Within is growing a service—not building a moat.</strong></div>
        </div>
      </section>

      <section className="investor-final">
        <div className="shell investor-final-grid"><div><p className="eyebrow light">The north star</p><h2>Make “Build my CRM” simple because thousands of difficult lessons sit behind the button.</h2><p>That is the business CRM From Within is trying to build. The first installations determine whether it deserves to exist.</p></div><div><Link className="button button-coral" href="/">See the customer experience <span aria-hidden="true">↗</span></Link><a className="plain-link" href="#proof">Review the proof milestones ↑</a></div></div>
      </section>

      <footer className="investor-footer shell"><Link className="brand brand-reverse" href="/"><Image src="/crm-from-within-mark-reverse.svg" width={34} height={34} alt="" unoptimized /><span>CRM From Within</span></Link><p>Investor vision · August 2026 · Pre-customer thesis, not market validation.</p><div><Link href="/">Customer vision</Link><a href="#top">Back to top ↑</a></div></footer>
    </main>
  );
}
